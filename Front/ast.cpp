...
/*#warning NEW: STATESPACE SHIFTING*/
...
ASTTerm1_Root::makeCode(SubstCode *subst)
{
  ...
  univ = d2->get(0);
  if (symbolTable.lookupType(univ)!=Univname && symbolTable.lookupType(univ)!=Statespacename) {
    cout << "Error:";
    pos.printsource();
...
ASTTerm2_Inter::makeCode(SubstCode *subst)
{
  ...
  IdentList *univs = ident_union(symbolTable.lookupUnivs(ct1->var),
				 symbolTable.lookupUnivs(ct2->var));
  /*#warning CHANGED TO UNION (BECAUSE OF STATESPACES AS UNIVS)*/
...
ASTForm_Import::makeCode(SubstCode *subst)
{
  ...
  Deque<char*> *newFileVars = new Deque<char*>; // TODO: memory leak?
  for (Deque<char*>::iterator it = fileVars->begin(); it != fileVars->end(); ++it) {
    newFileVars->push_back(*it);
  }

  return codeTable->insert(new Code_Import(file, newFileVars, actuals, pos));
}
